...
Será utilizada a técnica de **Mocks** para isolar as ViewModels das dependências externas (Firebase).  
Os testes serão automatizados e executados via **CLI do Flutter**.  
...
## 5. Fluxo de Execução  
- **Unidade:** Testar métodos de login e cadastro isoladamente  
- **Estado:** Verificar se `isLoading` muda para `true` durante as chamadas  
- **Integração:** Simular o fluxo do usuário clicando no botão e chegando na Dashboard  
